...
const loadShop=():ShopState=>{try{const saved=JSON.parse(localStorage.getItem(SHOP_KEY)??'null') as ShopState|null;return saved??{balance:100,lastReward:Date.now(),owned:[]};}catch{return{balance:100,lastReward:Date.now(),owned:[]};}};
...
export function CareerHome({ career, onPlay, onEditPlayer, onReset, onExit, onUpdateCareer }: { career: Career; onPlay: (match: MatchInfo) => void; onEditPlayer: () => void; onReset: () => void; onExit: () => void; onUpdateCareer: (career: Career) => void }) {
  const computerMode=localStorage.getItem('football-device')==='computer';
  ...
  const topScorers=[...([{name:'Лионель Месси',team:'Аргентина 🇦🇷'},{name:'Килиан Мбаппе',team:'Франция 🇫🇷'},{name:'Харри Кейн',team:'Англия 🏴'}].map((scorer)=>({...scorer,goals:simulatedScorerGoals(scorer.name),isPlayer:false}))),{name:career.player.name,team:career.player.country,goals:career.goals,isPlayer:true}].sort((a,b)=>b.goals-a.goals||Number(b.isPlayer)-Number(a.isPlayer)).slice(0,3);
  ...
  return <section className={`career-page ${computerMode?'career-page--computer':'career-page--phone'}`}>
    ...
    <div className="career-layout">
      <div className="profile-column"><button className="edit-player-button panel" onClick={onEditPlayer}>✏️ ИЗМЕНИТЬ МОЕГО ИГРОКА</button><aside className="profile panel">
        ...
        <div className="country country-with-flag"><img src={teamFlagUrl(career.player.country)} alt="" />{cleanTeamName(career.player.country)}</div>
      ...
      <main className="season">
        ...
        <div className="match-summary-row">
          ...
          {current ? <button className="next-world-match panel" onClick={() => setSelected(current)}><small>ИГРАТЬ</small><b><img src={teamFlagUrl(career.player.country)} alt="" />{cleanTeamName(career.player.country)} <span>VS</span> <img src={teamFlagUrl(current.opponent)} alt="" />{cleanTeamName(current.opponent)}</b><em>{current.tournament}</em></button> : <div className="notice win">Турнир завершён!</div>}
        </div>
    ...
    {shopOpen&&<div className="modal-backdrop" onClick={()=>setShopOpen(false)}><div className="shop-modal panel" onClick={(event)=>event.stopPropagation()}><button className="close" onClick={()=>setShopOpen(false)}>×</button><small>МАГАЗИН ИГРОКА</small><h1>{shop.balance} €</h1><div className="shop-grid">{[
      ['hair-fade','Модный фейд',100,{hair:'fade'}],['hair-afro','Большое афро',150,{hair:'afro'}],
